...
public final class CronCreateTool implements ToolDefinition<CronCreateTool.Input, CronCreateTool.Output> {
    ...
    public Output execute(Input input, ToolContext context) throws Exception {
        ...
        long intervalMs = parseIntervalMs(input.expression());
    ...
    static long parseIntervalMs(String expression) {
        ...
        if (trimmed.startsWith("@every ")) {
            return parseIntervalMs(trimmed.substring(7).strip());
        }
        ...
        if (parts.length == 5) {
            ...
            if (!minutePart.equals("*")) {
                try {
                    return Long.parseLong(minutePart) * 60_000L;
